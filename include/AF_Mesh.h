...
static inline AF_CMesh AF_Mesh_Load_Data(void* _filePtr, AF_CMesh _mesh){
  AF_CMesh returnMesh = {
    ...
    FILE* _file = (FILE*)_filePtr;
    if(_file == NULL){
        AF_Log_Warning("AF_Mesh_Load_Data: _file is null\n");
        return returnMesh;
    }
    ...
    if (_mesh.vertexCount <= 0 || _mesh.indexCount <= 0) {
        AF_Log_Warning("AF_Mesh_Load_Data: Invalid vertexCount or indexCount\n");
        return returnMesh;
    }
    ...
    AF_Vertex* vertexData = (AF_Vertex*)malloc(sizeof(AF_Vertex) * _mesh.vertexCount);
    if(vertexData == NULL){
        AF_Log_Warning("AF_Mesh_Load_Data: Failed to malloc vertex data\n");
        free(vertexData);
        return returnMesh;
    }
    uint32_t* indexData = (uint32_t*)malloc(sizeof(uint32_t) * _mesh.indexCount);
    if(indexData == NULL){
        AF_Log_Warning("AF_Mesh_Load_Data: Failed to malloc index data\n");
        free(indexData);
        return returnMesh;
    }
    ...
    while(!feof(_file)){
        ...
        // print the return value (aka string read in) to terminal
            ...
            if(fileBuffer[0] == 'v' && fileBuffer[1] == ' '){
                ...
                if (verticesCount >= _mesh.vertexCount) {
                    //AF_Log_Warning("AF_Mesh_Load_Data: Vertex count %u exceeds allocated size %u \n", verticesCount, returnMesh.vertexCount);
            ...
            if(fileBuffer[0] == 'f'){
                ...
                while(p != NULL) {
                    if((strcmp(p,"f"))){
                        ...
                        if (indicesCount >= _mesh.indexCount) {
                            //AF_Log_Warning("AF_Mesh_Load_Data: Index count exceeds allocated size\n");
                            break;  // Or handle as needed
                        }else{
            ...
            AF_Log_Warning("AF_Mesh_Load_Data: disabled texture coords and normals in mesh load \n");
            /*
            if(fileBuffer[0] == 'v' && fileBuffer[1] == 't'){
                // texture coordinate
...
static inline AF_CMesh AF_Mesh_Load_OBJ(const char* _filePath){
  AF_CMesh returnMesh = {
	...
	.material = {0,0,{0,0,0,0}},
	.showDebug = FALSE, 
    .meshType = AF_MESH_TYPE_MESH,
	.meshPath = _filePath,
	.meshID = 0,	// only fit 255 mesh types
	.isAnimating = FALSE,
	.modelMatrix = NULL,
	.displayListBuffer = NULL
    };
    ...
    if(file == NULL){
        AF_Log_Warning("%s Failed to open file %s\n", AF_MESH_FILE_TITLE, _filePath);
        return returnMesh;
    }
