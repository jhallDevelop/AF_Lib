...
af_bool_t AF_MeshLoad_FromFile(AF_Assets* _assets, AF_CMesh* _meshComponent, const char* _modelPath) {
    if(AF_File_Exists(_modelPath) == AF_FALSE){
...
af_bool_t AF_MeshLoad_Assimp(AF_Assets& _assets, AF_CMesh& _meshComponent, const char* _modelPath){
    ...
    AF_TextureLoader_SetFlipImage(_meshComponent.isImageFlipped);
    ...
    if(numMeshes > AF_MAX_MESH_COUNT){
    ...
    if (_meshComponent.material.diffuseTexture.type != AF_TEXTURE_TYPE_NONE)
    {
        // Reload texture\n");
        // Diffuse
        AF_TextureLoader_ReLoadTexture(&_assets, &_meshComponent.material.diffuseTexture);
        // Normal
        //_meshData.material.normalTexture = AF_TextureLoader_ReLoadTexture(&_assets, _meshData.material.normalTexture.path);
        // Specular
        //_meshData.material.specularTexture = AF_TextureLoader_ReLoadTexture(&_assets, _meshData.material.specularTexture.path);
    }
...
void AF_MeshLoad_Assimp_ProcessMesh(AF_Assets& _assets, const char* _meshPath, AF_MeshData& _meshData, aiMesh& _mesh, const aiScene& _scene, AF_Material* _material){
    ...
    if(_material->diffuseTexture.type != AF_TEXTURE_TYPE_NONE)
    {
        ...
        // Diffuse
        //_meshData.material.diffuseTexture = AF_TextureLoader_ReLoadTexture(&_assets, _meshData.material.diffuseTexture.path);
        // Normal
        //_meshData.material.normalTexture = AF_TextureLoader_ReLoadTexture(&_assets, _meshData.material.normalTexture.path);
        // Specular
        //_meshData.material.specularTexture = AF_TextureLoader_ReLoadTexture(&_assets, _meshData.material.specularTexture.path);
        ...
        //AF_TextureLoader_SetFlipImage(_meshData.)
...
AF_Texture AF_MeshLoad_Assimp_LoadMaterialTextures(AF_Assets& _assets, const char* _meshPath, aiMaterial& _assimpMat, aiTextureType _assimpType){
    ...
    for(unsigned int i = 0; i < assimpMaterialTextureCount; i++)
    {
        ...
        {   // if texture hasn't been loaded already, load it
            ...
            returnTexture.id = AF_TextureLoader_LoadTexture(returnTexture.path);
