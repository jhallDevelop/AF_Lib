...
================================
*/
void AF_File_ListFiles(const char *_path, AF_FileList* _fileList, af_bool_t _isAlphabetical)
{
#ifdef _WIN32
    ...
    // Three characters are for the "}\*" plus NULL append below

    //hFind = FindFirstFile(szDir, &ffd);
	char szDir[MAX_PATH] = "\0";
    snprintf(szDir, MAX_FILELIST_BUFFER_SIZE, "%s\\*", _path); // Append "\ * " to the path to search for files
    hFind = FindFirstFile(szDir, &ffd);

    if (INVALID_HANDLE_VALUE == hFind) {
        AF_Log_Error("AF_File_ListFiles: FindFirstFile failed for path: %s (Error %lu)\n", _path, GetLastError());
        return; // Exit the function if the path is invalid
    }

    do {
        // 3. FIX: Check for and ignore '.' and '..'
        if (strcmp(ffd.cFileName, ".") == 0 || strcmp(ffd.cFileName, "..") == 0) {
            continue; // Skip to the next file
        }

        if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            AF_Log("AF_File_ListFiles: Directory: %s\n", ffd.cFileName);
            // 4. FIX: Here you would add the directory to your _fileList
            // e.g., FileList_Add(_fileList, ffd.cFileName, 0, TRUE);
        }
        else {
            filesize.LowPart = ffd.nFileSizeLow;
            filesize.HighPart = ffd.nFileSizeHigh;
            AF_Log("AF_File_ListFiles: %s %lld bytes\n", ffd.cFileName, filesize.QuadPart);
            // 4. FIX: Here you would add the file to your _fileList
            // e.g., FileList_Add(_fileList, ffd.cFileName, filesize.QuadPart, FALSE);
        }
